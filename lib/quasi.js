...
exports.svg = function (opt) {
    opt = opt || {};
    var strokeWidth = opt.strokeWidth || "0.1";
    
    // SVG header
    var ret = [
            '<svg xmlns="http://www.w3.org/2000/svg" width="766" height="567" viewBox="0 0 766 567">'
        ,   '  <g transform="matrix(0 28.3465 -28.3465 0 609.449 28.6299)" stroke-width="' + strokeWidth + '">'
        ]
    ,   curObj = {
            commands: []
        }
    ...
    ,   {
            newPath:    function () {
                var str = '<path d="' + curObj.comands.join(" ") + '"';
        ...
        ,   moveTo:     function (x, y) {
                curObj.comands.push("m " + x.toFixed(2) + ", " + y.toFixed(2));
            }
        ,   lineTo:     function (x, y) {
                curObj.comands.push("l " + x.toFixed(2) + ", " + y.toFixed(2));
            }
        ,   closePath:  function () {
                curObj.comands.push("z");
            }
        ,   startGroup:  function () {}
        ,   endGroup:  function () {}
        ,   setGrey:  function (colour) {
                var col = (colour * 255).toString(16);
