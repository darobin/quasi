...
// XXX TODO
//  - add support for colours from http://www.physics.emory.edu/faculty/weeks/software/quasi_colour.c
//  - remove _i, _d, etc.
//  - split into separate modules
//  - browserify the SVG part
//  - canvas support as well
//  - document
//      - note that it could support canvas
//      - note that it is full of magic numbers and could use some love
//  - add a few demos to the repo
//  - investigate the weird rendering on the z examples
//  - N5 or S14 produce stroke=undefined
//  - npm
//  - ship
...
exports.quasi = function (opt, writer) {
    ...
    ,   plot = function (x_d, y_d, color_d, plotflag_i)  {
            ...
            if (dx_d < 1.3 && dy_d < 1 && dx_d > 0 && dy_d > 0) {
                ...
                minx = Math.min(minx, cmx_d);
                maxx = Math.max(maxx, cmx_d);
                miny = Math.min(miny, cmy_d);
                maxy = Math.max(maxy, cmy_d);
                if (plotflag_i < 1) {
                    writer.newPath();
    ...
    ,   segment = function (x1_d, y1_d, x2_d, y2_d, color_d) {
            plot(x1_d, y1_d, color_d, 0);
            plot(x2_d, y2_d, color_d, 2);
        }
    ;
                    ...
                    for (var t = 0; t < symmetry_i - 1; t++) {
                        for (var r = t + 1; r < symmetry_i; r++) {
                            ...
                            if (!flag_b) {
                                ...
                                plot(x0_d, y0_d, color_d, 0);
                                x0_d += (vx_d[t] || 0);
                                y0_d += (vy_d[t] || 0);
                                plot(x0_d, y0_d, color_d, 1);
                                x0_d += (vx_d[r] || 0);
                                y0_d += (vy_d[r] || 0);
                                plot(x0_d, y0_d, color_d, 1);
                                x0_d -= (vx_d[t] || 0);
                                ...
                                if (midon_i[0] > 0) {
                                    ...
                                    if (segtype_i === 3) {
                                        segment(midx1_d, midy1_d, midx3_d, midy3_d, color_d);
                                        segment(midx2_d, midy2_d, midx4_d, midy4_d, color_d);
                                    }
                                    else if (segtype_i === 1) {
                                        segment(midx1_d, midy1_d, midx2_d, midy2_d, color_d);
                                        segment(midx3_d, midy3_d, midx4_d, midy4_d, color_d);
                                    }
                                    else if (segtype_i === 2) {
                                        segment(midx1_d, midy1_d, midx4_d, midy4_d, color_d);
                                        segment(midx2_d, midy2_d, midx3_d, midy3_d, color_d);
                                    }
                                    else if (segtype_i === 4)  {
                                        segment(midx1_d, midy1_d, midx2_d, midy2_d, color_d);
                                        segment(midx3_d, midy3_d, midx4_d, midy4_d, color_d);
